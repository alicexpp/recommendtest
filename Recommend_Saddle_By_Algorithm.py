...
def select_saddle_date(table_name, recommend_rect_center, area_name):
    ...
    sql = "SELECT MIN(X_CENTER),MIN(Y_CENTER) FROM %s WHERE Y_CENTER=(SELECT MIN(Y_CENTER) FROM %s WHERE" \
          " Y_CENTER >= %d  AND X_CENTER >= %d AND HAS_COIL = 0 AND AREA_NAME='%s') AND X_CENTER >= %d AND HAS_COIL=0" \
          " AND AREA_NAME = '%s'" % (table_name, table_name, y_center, x_center, area_name, x_center, area_name)
...
def update_has_coil(table_name, x_center, y_center):
    conn.set_autocommit(True)
    cursor = conn.cursor()
    update_sql = "UPDATE %s SET HAS_COIL='%s' WHERE X_CENTER = %d AND Y_CENTER = %d" % \
                (table_name, '1', x_center, y_center)
...
def find_suit_pos(rect, rect_list, max_length, max_width, area_name, current_capacity):
    ...
    if not rect_x_list or not rect_y_list:
        ...
        if select_saddle != [(None, None)]:
            select_saddle_list.append(select_saddle)
        print "Finally select coordinate of area is:", rect_center
        print "Finally select saddle coordinate is:", select_saddle
    ...
    while i <= max_top:  # width direction
        ...
        while j <= max_right:  # length direction
            ...
            # 矩形不相交
                ...
                # 需要将其x,y坐标转换为大区域的坐标，再去选鞍座号，相对坐标转换成大区绝对坐标
                CENTER_X = Area_Coordinate_Trans.relative__to_absolute(area_name, copy_rect_center_x,
                                                                       copy_rect_center_y)[0]
                CENTER_Y = Area_Coordinate_Trans.relative__to_absolute(area_name, copy_rect_center_x,
                                                                       copy_rect_center_y)[1]
                # 计算出来的大区中心点坐标
                ...
                if select_saddle!=[(None, None)]:
                    ...
                    select_saddle_y = select_saddle[0][1]
                    # 将以库图中的鞍座坐标为中心点，转换成小区域的中心点，以便绘制rect
                    new_recommend_area_x = Area_Coordinate_Trans.absolute_to_relative(area_name, select_saddle_x,
                                                                                      select_saddle_y)[0]
                    new_recommend_area_y = Area_Coordinate_Trans.absolute_to_relative(area_name, select_saddle_x,
                                                                                      select_saddle_y)[1]
                    # 小区域中的矩形
                    new_recommend_rect = BaseClass.RECT(llp=BaseClass.POINT(new_recommend_area_x - float(width) / 2,
    ...
    if len(select_saddle_list) != 0:
        ...
        print "Finally select coordinate of area is:", (max_ratio_rect.center.x, max_ratio_rect.center.y)
        # 将小区中的矩形坐标转换成大区的鞍座坐标
        saddle_x_center = Area_Coordinate_Trans.relative__to_absolute(area_name, recommend_x_coordinate,
                                                                      recommend_y_coordinate)[0]
        saddle_y_center = Area_Coordinate_Trans.relative__to_absolute(area_name, recommend_x_coordinate,
                                                                      recommend_y_coordinate)[1]
        print "Finally select saddle coordinate is: ",(saddle_x_center, saddle_y_center)
